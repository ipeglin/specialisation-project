...
- End-to-end with actual fMRIPrep data
...
**New Files** (5):
1. `tcp/preprocessing/config/__init__.py`
2. `tcp/preprocessing/config/data_source_config.py`
3. `tcp/preprocessing/fmriprep_parcellation.py`
4. `tcp/preprocessing/README_FMRIPREP.md`
5. `CHANGELOG_FMRIPREP.md`

**Modified Files** (3):
1. `config/paths.py` - Added `get_parcellations_path()`
2. `config/default_config.json` - Added parcellations path
3. `tcp/preprocessing/run_pipeline.py` - Integrated fMRIPrep support
...
- 100 subjects (4 jobs): ~1-3 hours
- 500 subjects (4 jobs): ~5-15 hours

**Resource Requirements**:
- RAM: ~2-4 GB per subject, ~8-16 GB for 4 parallel jobs
- Disk: ~5-10 MB per subject .h5 file
- CPU: Benefits from multi-core (tested with 4 cores)

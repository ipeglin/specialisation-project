...
from .utils.validation import validate_file_paths, validate_manifest
...
class DataLoader:
    ...
    def resolve_file_path(self, file_path: str) -> Path:
        ...
        Supports both:
        - Relative paths (datalad mode): joined with base_path
        - Absolute paths (fMRIPrep mode): used directly

        Args:
            file_path: Path from manifest (relative or absolute)

        Returns:
            Resolved absolute path

        Raises:
            ManifestError: If base_path not configured for relative paths
        """
        path_obj = Path(file_path)

        # Check if path is absolute (works cross-platform)
        if path_obj.is_absolute():
            # Absolute path: use directly (fMRIPrep mode)
            return path_obj.resolve()
        else:
            # Relative path: join with base_path (datalad mode)
            if not self.base_path:
                raise ManifestError(
                    f"Cannot resolve relative path '{file_path}': no base path configured"
                )
            return resolve_platform_path(self.base_path, file_path)
